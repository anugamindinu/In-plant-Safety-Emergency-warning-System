    ...
    if (digitalRead(buttonPin[i]) == LOW) { // Button is pressed
      ...
      if (buttonPressed[i]) { // If the button was previously pressed
        ...
        if (pressDuration < longPressDuration) {
          ...
          if (i == 0) {
            message = "{\"Section\": \"section 3\", \"status\": \"Low Risk\", \"priority\": \"medium\"}";
            ...
            message = "{\"Section\": \"section 3\"}";
            clearLCD();
            displayMessage("section 3", "Good Condition");
            digitalWrite(buzzer,LOW);
          }
        } else {
          // Long press: High Risk
          if (i == 0) {
            message = "{\"Section\": \"section 3\", \"status\": \"High Risk\", \"priority\": \"high\"}";
            clearLCD();
